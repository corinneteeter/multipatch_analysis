...
for ii, pair in enumerate(all_pairs):
    ...
    for stim_key in pr_dict.keys():
        if stim_key != stim_to_get:
            ...
            if first_time_through:
                ...
                # specify cell classes taking into account some neurons don't have a cre but are differentiated by layer
                pre_class = specify_class(pair.experiment.slice.species,  pair.pre_cell.cre_type, pair.pre_cell.target_layer)
                post_class = specify_class(pair.experiment.slice.species,  pair.post_cell.cre_type, pair.post_cell.target_layer)
                if pre_class == '?':
                    logging.debug('something went wrong with'+ str(pair)+': couldnt specify a pre synaptic catagory')
                if post_class == '?':
                    logging.debug('something went wrong with'+ str(pair)+': couldnt specify a post synaptic catagory')
    ...
    if bail_out_flag is None: 
        if has_at_least_one_value is True:
            ...
            logging.info('not saving pair ' +str(pair)+ ': no '+stim_to_get+'stimuli')
