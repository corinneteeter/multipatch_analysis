...
#        if pr.stim_pulse_id in ic_pulse_ids:# cell_id[3:]: # 
        if pr.stim_pulse.recording.patch_clamp_recording.clamp_mode == 'ic':
            #  --don't need qc for presynaptic neuron
...
#                legacy_spike_time = pr.stim_pulse.spikes[0].max_dvdt_time      
    ...
    #            pre_inj_current = Trace(data=pr.stim_pulse.data, t0= start_time-legacy_spike_time+time_before_pulse, sample_rate=db.default_sample_rate).time_slice(start=0, stop=None)
                ...
                ax1.plot(time*1.e3, pre_syn_voltages[-1]*1.e3, color = colors)

                if spike_index:
                    #t0_new = t0 - time[spike_index] + time_before_pulse
    ...
    ax1.set_title("%.3f, %i, %i \n old algorithm" % (cell_id[0], cell_id[1], cell_id[2]))
    ax2.set_title("new algorithm")
