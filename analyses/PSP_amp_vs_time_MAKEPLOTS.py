...
dictionary=ju.read("PSP_vs_time_output_data/6_12_2017.json")
for syn_type in dictionary.keys():
    for key1 in dictionary[syn_type].keys():
        if key1 != 'num_of_synapses' and key1 != 'slopes' and key1 != 'intercepts' and key1 != 'f_pvalues' and key1 != 'max_residuals':
            print key1
...
#--------------------------------------------------------------------------------------------------------------------------------------------
#--------------------looking at the p value that the f-statistic has come from another distribution (want it to be low-----------------------
#--------------------------------------------------------------------------------------------------------------------------------------------

plt.figure()
for syn_type in dictionary.keys():   
    plt.hist(dictionary[syn_type]['f_pvalues']['sec_since_t0']['correct_amp_good_HP'])
...
for key3 in ['sweep_numbers', 'sec_since_t0']:
