...
class AverageFirstPulseFitPipelineModule4(DatabasePipelineModule):
    ...
    table_group = db.avg_first_pulse_fit_table4
        ...
        for (pre_cell_id, post_cell_id), pair in expt.pairs.items():
            try:
                ...
                if result['error'] is not None:
                    ...
                    result.pop('error')
                    afpf = db.AvgFirstPulseFit4(pair=pair, **result)
                    session.add(afpf)
            except:
    ...
    def job_records(cls, job_ids, session):
        ...
        """
        q = session.query(db.AvgFirstPulseFit4)
        q = q.filter(db.AvgFirstPulseFit4.pair_id==db.Pair.id)
